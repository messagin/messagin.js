...
export class ChatManager extends BaseManager<Chat> {
  ...
  async create(options: ChatCreateOptions) {
    ...
    const chat = new Chat(res.data, this.space!);
    this.store.set(chat.id, chat);
    return chat;
  }

  async fetch(id: string, force = false): Promise<Chat | null> {
    if (force || !this.store.has(id)) {
      ...
      this.store.set(id, new Chat(res.data, space!));
    }
    return this.store.get(id)!;
  }

  // todo implement fetchAll (chats)
  async fetchAll(): Promise<Chat[] | null> {
    ...
    for (const chat of res.data) {
      this.store.set(chat.id, new Chat(chat, this.space));
    }
